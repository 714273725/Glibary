...
public class GRefresher extends RelativeLayout {
    ...
    //刷新头的高度
    int mRefreshHeaderHeight = ScreenTools.px2dp(120);
    //回弹速度 1秒内回弹
    int mSpeed = 1;
   ...
   /* //波浪的高度,最大扩展高度
    ...
    protected void onAttachedToWindow() {
        ...
        if (mRefreshHeader == null) {
            mRefreshHeader = new FrameLayout(getContext());
            LayoutParams layoutParams = new LayoutParams(LayoutParams.MATCH_PARENT, 80);
            layoutParams.addRule(ALIGN_PARENT_TOP);
            layoutParams.addRule(CENTER_VERTICAL);
            mRefreshHeader.setLayoutParams(layoutParams);
            this.addView(mRefreshHeader);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(dx) <= Math.abs(dy)) {//滑动允许最大角度为45度
                    ...
                    if (!ScrollUtils.canChildScrollUp(mChildView) && ScrollUtils.canChildScrollDown(mChildView)) {
                        ...
                        //dy大于0，往下拉，小于0往上拉，不消费事件
                        if (mState == ScrollState.Normal) {
                            return dy > 0;
                        }

                        /*if ((mState == ScrollState.Normal || mState == ScrollState.PullingDown)) {
                            mState = ScrollState.PullingDown;
                            return true;
                        }
                        if (mState == ScrollState.Refreshing && dy < 0) {
                            mState = ScrollState.UpwardResilience;
                            mHandler.post(mUpwardSlideTask = new UpwardSlide());
                            return true;
                        }*/
                    } else if (!ScrollUtils.canChildScrollDown(mChildView) && ScrollUtils.canChildScrollUp(mChildView)) {
                        return false;
                    } else {
                        return false;
                    }
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                if (mState == ScrollState.UpwardResilience) {
                    ...
                    float t;
                    if (Math.abs(dy) > 600) {
                        t = mChildView.getTranslationY() - mRefreshHeaderHeight;
                    } else {
                        t = mChildView.getTranslationY() - mRefreshHeaderHeight / 10;
                    }
                    if (t <= 0) {
                ...
                if (dy > 0) {
                    ...
                    if (mState == ScrollState.Normal || mState == ScrollState.PullingDown) {
                        ...
                        if (dy > mRefreshHeaderHeight) {
                            mChildView.setTranslationY(mRefreshHeaderHeight / 2);
                            mState = ScrollState.Refreshing;
                            mHandler.removeCallbacks(mUpwardSlideTask);
                            mHandler.postDelayed(mUpwardSlideTask = new UpwardSlide(), 2000);
                        } else {
                            mChildView.setTranslationY(dy / 2);
                        }
                        return true;
                    }
                }
    ...
    public class UpwardSlide implements Runnable {
        @Override
        public void run() {
            float position = mChildView.getTranslationY() - mRefreshHeaderHeight / 8;
            if (position < 0) {
                mChildView.setTranslationY(0);
                mState = ScrollState.Normal;
                mHandler.removeCallbacks(mUpwardSlideTask);
                ...
                mState = ScrollState.UpwardResilience;
                mChildView.setTranslationY(position);
